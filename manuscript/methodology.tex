...
The iterative method proposed by \citet{siqueira-etal2017} (Algorithm \ref{alg:SOB17}) requires an entrywise product (line $11$), a half saxpy 
(line $12$) and a saxpy (lines $13$ and $14$). Then, we get from table \ref{tab:standard-flops} that the total number of flops is given by:
\begin{equation}
	f_{\mathtt{SOB17}} = 2D + \mathtt{ITMAX} \left( 2D^{2} + 3D \right) \: .
	\label{flops:SOB17}
\end{equation}
